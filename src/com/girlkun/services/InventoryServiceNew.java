...
public class InventoryServiceNew {
    ...
    public void addItemGiftCodeToPlayer(Player p, MaQuaTang giftcode) {
        ...
        try {
            JSONArray items = (JSONArray) JSONValue.parse(giftcode.detail);
            for (int i = 0; i < items.size(); i++) {
                JSONArray item = (JSONArray) items.get(i);
                int itemId = Integer.parseInt(item.get(0).toString());
                int quantity = Integer.parseInt(item.get(1).toString());
                if (itemId == -1) {
                    ...
                    JSONArray itemOptions = (JSONArray) item.get(2);
                    if (itemOptions.size() > 0) {
                        for (int j = 0; j < itemOptions.size(); j++) {
                            JSONArray itemOption = (JSONArray) itemOptions.get(j);
                            it.itemOptions.add(new Item.ItemOption(
                                    Integer.parseInt(itemOption.get(0).toString()),
                                    Integer.parseInt(itemOption.get(1).toString())));
                        }

                    }
                    addItemBag(p, it);
                    textGift += "x" + quantity + " " + it.template.name + "\b";
                }
            }
            sendItemBags(p);
...
public void addItemGiftCodeToPlayer1(Player p, MaQuaTang1 giftcode) {
        String textGift = "Bạn vừa nhận được:\b";
        try {
            JSONArray items = (JSONArray) JSONValue.parse(giftcode.detail);
            for (int i = 0; i < items.size(); i++) {
    ...
    private Item putItemBody(Player player, Item item) {
        ...
        int index = -1;
        if(player.isPet && index >7){
            Service.getInstance().sendThongBaoOK(player.isPet ? ((Pet) player).master : player, "Khong mang dc cho de tu");
            return sItem;
        }
        switch (item.template.type) {
            case 0:
    ...
    public void itemBagToBody(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            PlayerService.gI().sendPlayerAura(player);
    ...
    public void itemBodyToBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
             if (index == 11) {
                EffectSkillService.gI().sendEffectPlayerNew(player, false, (item.template.id - 1285) + 61);
            }
            if (index == 10) {

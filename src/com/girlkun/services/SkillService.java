...
public class SkillService {
    ...
    public boolean useSkill(Player player, Player plTarget, Mob mobTarget, Message message) {
        ...
        if(player.playerSkill.skillSelect == null){
            Service.gI().sendThongBao(player, "Chưa học skill");
            return false;
        }
        ...
        if (!player.isBoss) {
            if (player.isPet) {
                ...
                if (player.nPoint.stamina > 0) {
                    ...
                    Service.gI().sendThongBao(player, "Không đủ thể lực");
                    return;
                }
            }
        }
                    ...
                    for (Player pl : playersMap) {
                        if (pl != null && !player.equals(pl) && !player.nPoint.khangTDHS) {
                            if (Util.getDistance(player, pl) <= SkillUtil.getRangeStun(player.playerSkill.skillSelect.point) //                                    && canAttackPlayer(player, pl) //&& (!pl.playerSkill.prepareQCKK && !pl.playerSkill.prepareLaze && !pl.playerSkill.prepareTuSat)
                                ...
                                if (player.isPet && ((Pet) player).master.equals(pl) || pl.typePk == ConstPlayer.NON_PK) {
                                    continue;
                                }
            ...
            case Skill.HUYT_SAO:
                ...
                if (player.zone != null ) {
                    ...
                    if (!MapService.gI().isMapOffline(player.zone.map.mapId)) {
                        ...
                        List<Player> playersMap = new ArrayList<>(player.zone.getHumanoids());
                        synchronized (playersMap) {
                            for (Player pl : playersMap) {
                                synchronized (pl) {
                                    if (pl != null && (pl.isBoss || pl.isNewPet)) {
                                        continue;
                                    }
                                    if (pl != null) {
                                        ...
                                        if (!pl.isBoss && !pl.isNewPet && pl.gender != ConstPlayer.NAMEC
